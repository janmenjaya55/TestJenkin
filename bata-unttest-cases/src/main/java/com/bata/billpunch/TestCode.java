...
public class TestCode {
	public static void main(String[] args) {
		...
		// StringBuffer str2 = new StringBuffer();//functionaly same only its
		// synchronised.
		str2.append(str);
		str2 = str2.reverse(); // used string builder to reverse
		System.out.println(
				"Write a Java Program to reverse a string with using String inbuilt function>>>>>>>>>>>" + str2);
		...
		System.out.println("Java Program to remove all white spaces from a string without using replace()" + sb); // Output
																													// :
																													// CoreJavajspservletsjdbcstrutshibernatespring

		// Write a Java Program for the Fibonacci series. means next 2 value sum will be
		// 3rd num
		// 1, 1, 2, 3, 5, 8, 13
		...
		// programm for INPUT_STRING = "Abcdedfavafdalafad"; // OUTPUT : Abcde, dfav,
		// afd, al,af,ad

		String xm = "Abcdedfavafdalafad";
		List<String> list = new ArrayList<String>();
		char[] chm = xm.toCharArray();
		StringBuffer strbuf = new StringBuffer();
		for (int i = 0; i < xm.length(); i++) {
			strbuf.append(chm[i]);
			if (xm.length() - 1 != i) {
				if (strbuf.toString().contains(String.valueOf(chm[i + 1]))) {
		...
		System.out.println("final list of words for this above str>>>>>>>>>>>>>>>>>" + list);
	}
