...
public interface ArticlesMasterDao extends JpaRepository<ArticlesMasterModel, Long> {
	...
	public List<ArticlesMasterModel> findWithArticleDetails(String artno);
	
	@Query(nativeQuery = true, value = "select a.* from  tm_articles_master_dtls a where a.ART_NO like ?1 ")
	public ArticlesMasterModel findWithArticleDetailsByCode(String artcode);
	
	@Query(nativeQuery = true, value = "select round(a.ART_PRICE_MRP,2) as artpricemrp ,round(a.ART_PRICE_FACTORY,2) as artpricefactory,round(a.ART_STND_COST ,2)  as artstdCost ,a.ART_SEQUENCE_NO as artseq from  tm_articles_master_dtls a where a.ART_NO like ?1 ")
	public BillPunchResponseInterface findWithArticleDetailsByCodeReport(String artcode);
	
	@Query(nativeQuery = true, value = "select distinct round(a.ART_PRICE_WSL*?2,2) as artpricewspedp ,round(a.ART_PRICE_MRP*?2,2) as artpricemrpedp,round(a.ART_PRICE_FACTORY*?2 ,2)  as artpricefactoryedp ,round(a.art_stnd_cost*?2 ,2)  as artstdCostedp from  tm_articles_master_dtls a where a.ART_NO like ?1 ")
